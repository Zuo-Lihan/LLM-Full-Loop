...
def GPT_onetime_chat_message_score(prefix_txt, response, unrelated_words_txt, judge_messages_write_txt, data_name, api_KEY, base_url, model_name, temperature=1.0, logprobs=True, top_p=1.0):
    ...
    index = 2
    for i in range(contents_lens):
        if contents[i] == '':
            index = i
            break
    judgements = contents[index-1].replace(' ', '').split(',')

    explanations = []
    for expl_index in range(index, contents_lens):
    ...
    Score_rules_dict = {'True':int(1), 'False': int(-1), 'Neither': int(0), 'neither': int(0)}
